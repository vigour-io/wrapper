  ...
  if (this.builds) {
    ...
    function installIntoTemplate () {
      ...
      function injectGradle (pluginInfoList) {
        ...
        _.uniqBy(pluginInfoList, plugin => plugin.name)
      ...
      function injectManifest (permissions, metadata, activities) {
        ...
        return editXML(manifestPath, function (xml) {
          ...
          xml.manifest['uses-permission'] = _.chain(permissions)
            .map((permission) => ({
              '$': {
                'android:name': permission
              }
            }))
            .concat(xml.manifest['uses-permission'])
            .uniqBy((permission) => permission.$['android:name'])
            .value()
      ...
      pluginInfoList.forEach(function (info) {
        ...
        if (info.android.permissions) {
          permissions = permissions.concat(info.android.permissions)
        }
