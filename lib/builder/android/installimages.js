...
module.exports = exports = function () {
  if (this.builds) {
    ...
    return exists(cacheFilename)
      ...
      .then(function (cachedState) {
        ...
        if (self.splashScreen) {
          ...
          let src = path.join(self.root, self.splashScreen)
          pending.push(upToDate(src, 'splash')
            .then(function (isUpToDate) {
              if (isUpToDate) {
                log.info('skipping up to date image ' + self.splashScreen)
              } else {
                manips.push({
                  src: src,
                  batch: [{
        ...
        if (self.appIcon) {
          ...
          let src = path.join(self.root, self.appIcon)
    ...
    log.info('- skipping installing images -')
