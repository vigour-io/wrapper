...
Builder.prototype.start = function () {
  ...
  if (this.config.native.root.val) {
    return readJSON(path.join(this.config.native.root.val, 'package.json'))
      .then(function (pkg) {
        var merge = new Base(self.config.plain())
        merge.set(pkg)
...
Builder.prototype.nextStep = function (opts) {
  ...
  var customPlatform = options.native.customPlatform
  var platform
