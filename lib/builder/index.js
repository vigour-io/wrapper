...
Builder.prototype.start = function () {
  ...
  return readJSON(pkgPath)
    .then(function (pkg) {
      var configuredPlatforms = _keys(pkg.vigour.native.platforms)
            ...
            if (pkgPath) {
              return readJSON(pkgPath)
                .then(function (pkg) {
                  ...
                  // console.log("Can't read ", pkgPath, reason)
                  return false
                })
            } else {
              return false
...
function searchDependencies (dependencies, root) {
  if (!dependencies) {
    ...
    for (var key in dependencies) {
      pluginPkgPath = path.join(root, 'node_modules', key, 'package.json')
    ...
    return Promise.all(pluginPkgPaths.map(function (pkgPath) {
      return readJSON(pkgPath)
        ...
        .catch(function (reason) {
          // console.log('caught', pkgPath, reason)
          return false
        })
    }))
    .then(function (arr) {
