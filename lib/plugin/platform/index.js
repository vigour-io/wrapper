  ...
  _platform: new Observable({
    ...
    on: {
      ...
      ChildConstructor: new Emitter({
        define: {
          emitInternal (data, event) {
            console.log('------> thing emitInternal!')
            var platform = this.parent.parent
            var ready = platform.ready
            if (ready.val) {
              console.log('platform is ready! emitInternal!', platform.pluginId && platform.pluginId.val)
              console.log('platform emitInternal data', data, 'on', this.path)
              emitInternal.apply(this, arguments)
              console.log('did that apply lol')
            } else {
              console.log('platform is not ready! emitInternal init, and when ready, emitInternal this.')
              var emitter = this
              ...
              ready.is(truthy, function isReady (val, event) {
                ...
                console.log('platform emitInternal data', data)
                ready.off(isReady)
