  ...
  return opts.plugins.reduce(function (prev, pluginName) {
    return prev.then(function () {
      return new Promise(function (resolve, reject) {
        ...
        var pluginBuildScript = path.join(androidPath, 'build.js')
        console.log('   trying to exe: ',pluginBuildScript)
        var build = Promise.resolve()
        if ( fs.existsSync(pluginBuildScript)) {
          build = exe(pluginBuildScript, '')
        }
        resolve(build.then(retrievePluginInfo))
      })
    })
  }, Promise.resolve()).then(installIntoTemplate)
}
