...
function customizeTemplate (opts) {
  ...
  if (opts.appIndexPath) {
    ...
    javaFile = javaFile.replace('file:///android_asset/index.html',
      'file:///android_asset/' + opts.appIndexPath)
    fs.writeFileSync(filename, javaFile)
  }

  if (opts.productName) {
    log.info('set app name to ', opts.productName)
    promises.push(editXML(path.join(opts.srcDir, 'res', 'values', 'strings.xml'),
      function (xml) {
  ...
  if (opts.splashDuration) {
    log.info('set splash duration to ', opts.splashDuration)
    promises.push(editXML(path.join(opts.srcDir, 'res', 'values', 'integers.xml'),
      function (xml) {
