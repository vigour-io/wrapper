...
function buildCore (path) {
  return exports.exe('./gradlew plugincore:uploadArchives', path)
}
  ...
  return Promise.resolve()
    .then(function () {
      if (opts.keystoreFile) {
        return Promise.resolve(opts.keystoreFile)
          .then(fs.readFile)
          .then(function (keystore) {
            var keystorePath = path.join(opts.buildDir, 'release.keystore')
            return fs.writeFile(keystorePath, keystore)
          })
      } else {
        return Promise.resolve()
      }
