...
exports.installTemplate = function installTemplate (opts) {
...
function runOnDevice (opts) {
  log.info('start run')
  return exe(opts.adbPath + ' shell monkey -p ' + opts.packageName + ' 1',
    opts.root)
    .then(function () {
      return opts
    })
...
exports.optionalRun = function optionalRun (opts) {
  if (!opts.run) {
    return opts
  }
...
function installPlugins (opts) {
  ...
  function installIntoTemplate () {
    return new Promise(function (resolve, reject) {
      ...
      var mainFile = 'java/io/vigour/nativewrapper/MainActivity.java'
      template.readFile(path.join(opts.srcDir, mainFile))
