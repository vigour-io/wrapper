...
var readJs = Promise.denodeify(fs.readJs)
var editXML = Promise.denodeify(fs.editXML)
var remove = Promise.denodeify(fs.remove)
var imgServer = require('vigour-img')
var getChecksum = Promise.denodeify(require('checksum'))
...
function installImages (opts) {
  ...
  var cachedState = {}
  if (cacheFile) {
    cachedState = JSON.parse(cacheFile)
  }
  log.info('read image cache', cachedState)

  function upToDate (imPath) {
    var sum = getChecksum(fs.readFileSync(imPath))
    var upToDate = cachedState.imPath === sum
  ...
  if (opts.appIcon) {
    ...
    var src = path.join(opts.root, opts.appIcon)
    if (!upToDate(src)) {
      manips.push({
  ...
  if (manips.length) {
    log.info('installing images: ', manips)
    return imgServer({
      convertPath: 'forceUseOfRemote',
      manip: manips
    }).then(function () {
      fs.writeFileSync(cacheFilename, JSON.stringify(cachedState))
      log.info('written image cache', cachedState)
      return opts
    })
  } else {
    return opts
