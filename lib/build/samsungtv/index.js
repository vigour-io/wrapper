  ...
  return new Promise(function (resolve, reject) {
    for(var file in parsed) {
      ...
      if (fs.existsSync(path.join(opts.root, parsed[file]))) {
        if(file === "ThumbIcon"){
          arr.push({src:path.join(opts.root, parsed[file]),dst: opts.buildDir + '/icons/' + file + ".png",width:106, height:86})
        }else if(file === "BigThumbIcon"){
          arr.push({src:path.join(opts.root, parsed[file]),dst: opts.buildDir + '/icons/' + file + ".png",width:115, height:95})
        }else if(file === "ListIcon"){
          arr.push({src:path.join(opts.root, parsed[file]),dst: opts.buildDir + '/icons/' + file + ".png",width:85, height:70})
        }else if(file === "BigListIcon"){
          arr.push({src:path.join(opts.root, parsed[file]),dst: opts.buildDir + '/icons/' + file + ".png",width:95, height:78})
        }
      }
      else {
