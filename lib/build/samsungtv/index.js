  ...
  return new Promise(function (resolve, reject) {
    for(var file in parsed) {
      if (fs.existsSync(parsed[file])) {
        if(file === "ThumbIcon"){
          arr.push({src:parsed[file],dst: opts.buildDir + '/icons/' + file + ".png",width:106, height:86})
        }else if(file === "BigThumbIcon"){
          arr.push({src:parsed[file],dst: opts.buildDir + '/icons/' + file + ".png",width:115, height:95})
        }else if(file === "ListIcon"){
          arr.push({src:parsed[file],dst: opts.buildDir + '/icons/' + file + ".png",width:85, height:70})
        }else if(file === "BigListIcon"){
          arr.push({src:parsed[file],dst: opts.buildDir + '/icons/' + file + ".png",width:95, height:78})
        }
      }
      else {
        log.error(" This path " + parsed[file] + " doesn't exist")
        throw "error"
      }
    }
