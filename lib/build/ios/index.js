...
function nativeAssets (opts) {
	...
	if(opts.splashScreen) {
		...
		var launchImagePath = path.join(xcodeAssetsPath, 'LaunchImage.launchimage')
		var splashScreen = path.join(opts.root, opts.splashScreen)

		var contentsFileContent = {}
		
		return _mkdirp(launchImagePath).then(function() {
			
			return _readJSON(launchImageTplPath).then(function(contentsObj) {
				
				contentsFileContent = contentsObj
				
				var imageDimensions = _.pluck(contentsFileContent.images, 'filename')
				var fileNames=[],l=imageDimensions.length;while(l--)fileNames[l]="splash-"+(l+1)+".png"
				
				var data = {}
				data.images = _.zipObject(imageDimensions, fileNames)
				data.splashScreen = splashScreen
				data.outputPath = launchImagePath
			...
			var contentsPath = path.join(launchImagePath, 'Contents.json')
	...
	//app icon
	if(opts.appIcon) {
			var appIconTplPath = path.join(__dirname, '..', '..', '..', 'lib', 'build', 'ios', 'appIconTpl.json')
	}
