...
function ready (err, response, pluginId) {
  if (err) {
    ...
    if (pluginId) {
      if (plugins[pluginId]) {
        plugins[pluginId].emit('ready', void 0, response)
      }
      ...
      exports.emit('ready', void 0, response)
    }
  }
}
...
function error (err, pluginId) {
  if (pluginId) {
    if (plugins[pluginId]) {
      plugins[pluginId].emit('error', void 0, err)
    } else {
      console.error('Error for unregistered plugin ' + pluginId, err)
    }
  } else {
    exports.emit('error', void 0, err)
  }
}
...
function receive (err, message, pluginId) {
  if (err) {
    ...
    if (pluginId) {
      plugins[pluginId].emit('receive', void 0, message)
    } else {
      exports.emit('receive', void 0, message)
    }
  }
}
